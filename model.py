...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    corpus = []
    for i in ' '.join(sentences).split():
        if i not in corpus and i not in specials:
            corpus.append(i)
    setences_to_dict = dict()
    for i in range(len(specials)):
        setences_to_dict[specials[i]] = i

    for i in range(len(corpus)):
        setences_to_dict[corpus[i]] = i+len(specials)
    
    return setences_to_dict
